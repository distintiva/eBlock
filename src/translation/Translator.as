...
//	private static const font13:Array = ['ar'];
	...
	public static function initializeLanguageList():void {
		...
		languages = eBlock.app.server.getLanguageList();
	...
	public static function setLanguage(lang:String):void {
		...
		setFontsFor(lang);
		if ('en' == lang){
			eBlock.app.translationChanged(); // there is no .po file English
		}else {
			var data:Object = eBlock.app.server.getPOFile(lang);
			if (data) {
				dictionary = data;
				checkBlockTranslations();
				setFontsFor(lang);
				eBlock.app.extensionManager.parseAllTranslators();
			}
			setTimeout(eBlock.app.translationChanged, 0);
//			MBlock_mod.app.translationChanged();
		}
		langChangedSignal.dispatchEvent(new Event(Event.CHANGE));

		eBlock.app.server.setSelectedLang(lang);
	}
	...
	private static function fontSizeMenu():void {
		...
		m.showOnStage(eBlock.app.stage);
	}
...
//		for each (var entry:Array in Specs.commands) checkBlockSpec(entry[0]);
	...
	private static function checkBlockSpec(spec:String):void {
		...
		if (!argsMatch(extractArgs(spec), extractArgs(translatedSpec))) {
			eBlock.app.log('Block argument mismatch:');
			eBlock.app.log('    ' + spec);
			eBlock.app.log('    ' + translatedSpec);
