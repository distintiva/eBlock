...
//			runtime.interp.redraw();
	...
	private function specForCmd():String {
		...
		if(i > -1) {
			var spec:Array = eBlock.app.extensionManager.specForCmd(cmd);
	...
	public function menu(evt:MouseEvent):Menu {
		function handleMenu(item:int):void {
			...
			if (item == 10) {
				visible = false;
				eBlock.app.updatePalette(false);
			}
		}
		if (!eBlock.app.editMode) return null;
...
//			m.addItem("text", 4);
	...
	private function mouseDown(evt:MouseEvent):void {
		...
		if (p.y > 20) eBlock(root).gh.setDragClient(this, evt);
	}
