...
//		MBlock_mod.app.interp.redraw();
	...
	private function addItem(b:IconButton = null):void {
		...
		if ((root is eBlock) && !(root as eBlock).editMode) return;
...
//		if (limitedCloudView &&
//			MBlock_mod.app.isLoggedIn() && MBlock_mod.app.editMode &&
//			(MBlock_mod.app.projectOwner == MBlock_mod.app.userName)) {
//				limitedCloudView = false; // only project owner can view cloud list contents
//		}
		var isEditable:Boolean = eBlock.app.editMode && !limitedView;
		updateElementCount();
		removeAllCells();
		visibleCells = [];
		visibleCellNums = [];
