...
package blocks {
	...
	import cc.customcode.uibot.uiwidgets.lightSetter.LightSensor;
...
public class BlockArg extends Sprite {
	...
	public function BlockArg(type:String, color:int, editable:Boolean = false, menuName:String = '') {
		...
		} else if(menuName == "drawPixels"){
		   base.setWidthAndTopHeight(LightSensor2.COUNT_W * LightSensor2.BMP_ICON_SCALE, LightSensor2.COUNT_H * LightSensor2.BMP_ICON_SCALE, true);
		...
		if ((type == 'd') || (type == 'm' && menuName != "drawFace" && menuName != "drawPixels") /*|| (type == 'm' && menuName != "drawPixels")*/ ) { // add a menu icon
			menuIcon = new Shape();
		...
		if(menuName == "drawFace"){
			defaultBmd = new BitmapData(16, 8, false, 0xFFFFFF);
			base.bmd = defaultBmd;
		...
		if(menuName == "drawPixels"){
			//defaultBmd = new BitmapData(LightSensor2.COUNT_W, LightSensor2.COUNT_W, false, 0xFFFFFF);
			//defaultBmd = new BitmapData(16, 16, false, 0xFFFFFF);
			//base.bmd = defaultBmd;
			base.bmd = LightSensor2.createDefaultBmp();
			
			//base.setWidthAndTopHeight(30, Block.argTextFormat.size + 5);
			if (editable) {
				base.setColor(0xFFFFFF); // if editable, set color to white
				_isEditable = true;
			}
		...
		if (  (menuName != "drawFace" && menuName != "drawPixels") && ( editable || isNumber || type == 'm')/* && menuName != "drawFace" && menuName != "drawPixels"*/ /*|| (type == 'm' && menuName != "drawPixels")*/) { // add a string field
	...
	static private var defaultBmd:BitmapData ; //= new BitmapData(16, 8, false, 0xFFFFFF);
	...
	private function textChanged(evt:*):void {
		...
		if (evt && eBlock.app) eBlock.app.setSaveNeeded();
		if(eBlock.app.stageIsArduino && !isShowingCode){
			isShowingCode = true;
