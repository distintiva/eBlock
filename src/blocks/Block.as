...
/*
	public function showHelp():void {
		...
		if((i = op.indexOf('.')) > -1) {
			...
			if(MBlock_mod.app.extensionManager.isInternal(extName))
				MBlock_mod.app.showTip('ext:'+extName);
			else
				DialogBox.notify('Help Missing', 'There is no documentation available for experimental extension "'+extName+'".', MBlock_mod.app.stage);
		}
		else {
			MBlock_mod.app.showTip(op);
		}
	}
*/
	public function duplicateStack(deltaX:Number, deltaY:Number):void {
		if (isProcDef()) return; // don't duplicate procedure definition
		var forStage:Boolean = eBlock.app.viewedObj() && eBlock.app.viewedObj().isStage;
		var newStack:Block = BlockIO.stringToStack(BlockIO.stackToString(this), forStage);
		newStack.x = x + deltaX;
		newStack.y = y + deltaY;
		parent.addChild(newStack);
		eBlock.app.gh.grabOnMouseUp(newStack);
	}
...
//		var top:Block = topBlock();
//		// TODO: Remove any waiting reporter data in the MBlock_mod.app.extensionManager
//		if (parent is Block) Block(parent).removeBlock(this);
...
//		if (top != this) x += top.width + 5;
//		removeComments();
//		MBlock_mod.app.runtime.recordForUndelete(this, x, y, 0, MBlock_mod.app.viewedObj());
//		MBlock_mod.app.scriptsPane.saveScripts();
//		MBlock_mod.app.runtime.checkForGraphicEffects();
//		return false;
	}
	private function removeComments():void{
		var comments:Array = attachedCommentsIn(eBlock.app.scriptsPane);
		...
		eBlock.app.scriptsPane.fixCommentLayout();
	}
	...
	public function objToGrab(evt:MouseEvent):Block {
		if (isEmbeddedParameter() || isInPalette()) return duplicate(false, eBlock.app.viewedObj() is ScratchStage);
		return this;
	}
	...
	public function click(evt:MouseEvent):void {
		...
		if(topBlock().op.indexOf("runArduino")>-1){
			if(!eBlock.app.stageIsArduino){
				eBlock.app.changeToArduinoMode();
			}
			return;
		}
		eBlock.app.runtime.interp.toggleThread(topBlock(), eBlock.app.viewedObj(), 1);
	}
