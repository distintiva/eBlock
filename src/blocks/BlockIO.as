...
public class BlockIO {
	...
	private static function arrayToBlock(cmd:Array, undefinedBlockType:String, forStage:Boolean = false):Block {
		...
		if (cmd[0] == 'getUserName') eBlock.app.usesUserNameBlock = true;
		...
		var blockArg:BlockArg;
		
		if(cmd[0].indexOf(".showDraw") > 0){
			args = cmd.slice(1,-1);
			//*JC* spec[4][3] es el argumento del dibujo
			if(spec[4] && spec[4][3])  
			{
				args.push(spec[4][3]);
			}
			...
			blockArg = b.args[0];   //*JC* args[0] es el argumento del dibujo
			blockArg.argValue = cmd[1] is Array ? cmd[1] : LightSensor2.defaultValue;  //*JC*  cmd = NombreBloque, argum1, argum2,...
			blockArg.base.bmd = LightSensor2.arrayToBmd(blockArg.argValue);
			blockArg.base.setColor(0);
			blockArg.base.redraw();
		} else{
	...
	public static function specForCmd(cmd:Array, undefinedBlockType:String):Array {
		...
		var extensionSpec:Array = eBlock.app.extensionManager.specForCmd(op);
		if (extensionSpec) return extensionSpec;

		var spec:String = 'Err: ' + cmd[0];
