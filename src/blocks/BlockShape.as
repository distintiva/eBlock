...
public class BlockShape extends Shape {
	...
	static private const CornerInset:int = 2;  //*JC* prev 3
	...
	public function redraw():void {
		...
		if(bmd != null){
			...
			//- creo un bitmapData mejorado con separación entre pixeles teniendo como base el Bitmapdata Original
			var bmdCopy:BitmapData = LightSensor2.getEnhancedBmd(bmd);
			
			var matrix:Matrix = new Matrix();
			var scaleValue:Number = topH / bmdCopy.height;
			
			
			scaleValue=bmdCopy.height>20 ?1:2 ; //*JC* hacemos los bloques de matriz de leds mas grandes cuando la matriz sea pequeña: ej5x5
			
			matrix.scale(scaleValue, scaleValue);
			
			
			
			g.beginBitmapFill(bmdCopy, matrix, false);
			
			//g.drawRect(0, 0, LightSensor.COUNT_W*scaleValue, LightSensor.COUNT_H*scaleValue); //*JC*
			g.drawRect(0, 0, bmdCopy.width*scaleValue, bmdCopy.height  *scaleValue);
			...
			g.lineStyle(1,   LightenDarkenColor(color,0.3)  );  //*JC*  le pongo una linea un poco más oscura que el color del bloque
	...
	//*JC* oscurece o aclara un color según el amt [-1.0 a +1.0 ]
	private function LightenDarkenColor(col:int, amt:*):int {
		//col = parseInt(col,16);
		var r:int = (col >> 16);
			r = r*(1-amt);
		...
		var g:int = ((col >> 8) & 0x00FF); 
			g =  g*(1-amt);
			
		
		
		var newColor:int = b | (g << 8) | (r << 16);
		return newColor;
	}
	...
	private function drawLoopArrow(g:Graphics, h:int):void {
		...
		//drawPath(g, w - 15, h - 3, arrow); // shadow *JC*
