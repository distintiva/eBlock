...
public class ExtensionManager {
	...
	public  function getBlocksOverride( ext: ScratchExtension ): Array{
		...
		//- añado los bloques que queden de la extensión base
		for each (spec in deviceBlocks) {
			if ( spec.length < 3 && spec[0]== 'category' || spec[0]=='-' ){
				...
				if( ext.removeFromBase.indexOf( blockName ) >=0  ) continue; // para no añadir los bloques marcados a eliminar en una extension heredada
			}
	...
	//<- ScratchRuntime	
		...
		//- no pertenece a una extensión  ¿?¿?  ojo,  revisar que los bloques de _base_  vengan con el prefijo
	...
	public function specForCmd(op:String):Array {
		...
		var blockCandidateInbase:Array=[]; //- por si no encontramos el bloque en su extension pero si está en base
		...
		//	trace(count++);
			for each (var spec:Array in ext.blockSpecs) {
				...
				if(ext.name =="_base_"){ // op siempre vendrá como  _base_.runDigital  y spec[2]  como _base_.runDigital
					if ( spec[2] == op) {
					...
					//- o que venga con prefijo de Device.runArduino y  la de _base_ esté tdavia sin prejijo  "runArduino
...
//		}
	...
	public function loadRawExtension(extObj:Object):void {
		...
		//- añado el prefijo de la extension a cada bloque
		/*for each (var spec:Array in ext.blockSpecs ) {
