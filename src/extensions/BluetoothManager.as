		...
		public function discover():void{
			if(_bt){
				...
				if(!_bt.isDiscovering){
					//eBlock.app.track("/OpenBluetooth");
		...
		public function open(port:String):Boolean{
			...
			function checkName():void{
				if(_bt.connected){
					...
					_isBusy = false;
					addBluetoothHistory();
					eBlock.app.topBarPart.setConnectedTitle("Bluetooth");
		...
		private function onDiscoverChanged(evt:Event):void{
			...
			for(var i:uint=0;i<_list.length;i++){
				for(var j:* in _dialogboxDiscover){
					...
					d.x = (eBlock.app.stage.stageWidth-d.width)/2<0?0:(eBlock.app.stage.stageWidth-d.width)/2;
					d.y = (eBlock.app.stage.stageHeight-d.height)/2<0?0:(eBlock.app.stage.stageHeight-d.height)/2;
				}
			}
