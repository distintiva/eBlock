...
public class ScriptsPane extends ScrollFrameContents {
	...
	public function ScriptsPane(app:eBlock) {
		this.app = app;
	...
	private function createTexture():void {
		const alpha:int = 0xFF << 24;
		const bgColor:int = alpha | 0xffFFFF; //*JC*
		const c1:int = alpha | 0xffFFFF;
		const c2:int = alpha | 0xffFFFF;
...
//			app.scriptsPart.showArduinoCode();
	...
	//- almacena las coordenadas del rectangulo que ocupa el objeto
	public function minRect():Rectangle{
		...
		for each(var b:Block in allStacks()){
			if( b.x<bx)	bx = b.x;
			if( b.x + b.width >bw)	bw = b.x + b.width;
			if( b.y<by)	by = b.y;
			if( b.y + b.height >bh)	bh = b.y + b.height;
			
		}
		
		return new Rectangle(bx, by, bw-bx, bh-by);
	}
...
return true; // xxx disable this check for now; it was causing confusion at Scratch@MIT conference
	...
	private function __onSelect(evt:Event):void
	{
		...
		if(evt.target.data &&  evt.target.data.@action =="save_blocks"){
			saveBlocksAsPng();		
