package cc.customcode.uibot.uiwidgets.lightSetter
{
	...
	internal class ThumbPane2 extends JPanel
	{
		...
		static internal const defaultBmd:BitmapData = new BitmapData(5, 5, false, 0xFFFFFF);
		
		static private const numW:int = 6;
		...
		private var frame:LightSetterFrame2;
		
		public function ThumbPane2(frame:LightSetterFrame2)
		{
			super(new BorderLayout(10, 10));
			this.frame = frame;
			var wrapper:JPanel = new JPanel(new CenterLayout());
			var dock:JPanel = new JPanel(new GridLayout(2, 6, 12, 4));
			for(var dy:int=0; dy<numH; dy++){
				for(var dx:int=0; dx<numW; dx++){
					var pane:AssetPane = new AssetPane(LightSensor2.createBmp(defaultBmd));
		...
		{
			btnLeft.setVisible (currentPage > 1);
			//btnLeft.setEnabled(currentPage > 1);
			//btnRight.setEnabled(currentPage < totalPages);
			btnRight.setVisible(currentPage < totalPages);
