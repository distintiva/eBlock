package cc.customcode.uibot.uiwidgets.extensionMgr
{
	...
	public class DefaultListCell extends AbstractListCell{
		...
		private function updataBtnStatus():void
		{
			if(ExtensionUtil.showType==0)
			{
				switch(hasDownloaded(dataObj,eBlock.app.extensionManager.extensionList))
				{
					case -1:
						//no download
			...
			else if(ExtensionUtil.showType==1)
			{
				...
				if(dataObj.isExtensionBoard && delBtn.parent)
				{
		...
		private function onDownloadComplete(e:Event):void
		{
			...
			//MBlock_mod.app.stage.removeChild(progressSp);
			trace("保存完成")
		}
		private function __onViewSource(evt:AWEvent=null):void
		{
			...
			var file:File = ApplicationManager.sharedManager().documents.resolvePath("appfiles/resources/extensions/"+path);
			if(file.exists){
