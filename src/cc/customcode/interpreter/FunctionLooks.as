...
package cc.customcode.interpreter {
	...
	private function primSceneIndex(thread:Thread, argList:Array):void {
		thread.push( eBlock.app.stagePane.costumeNumber());
	}

	private function primSceneName(thread:Thread, argList:Array):void {
		thread.push( eBlock.app.stagePane.currentCostume().costumeName);
	}

	private function startScene(s:String):Array {
		if ('next backdrop' == s) s = backdropNameAt(eBlock.app.stagePane.currentCostumeIndex + 1);
		else if ('previous backdrop' == s) s = backdropNameAt(eBlock.app.stagePane.currentCostumeIndex - 1);
		else {
			var n:Number = Interpreter.asNumber(s);
			if (!isNaN(n)) {
				n = (Math.round(n) - 1) % eBlock.app.stagePane.costumes.length;
				if (n < 0) n += eBlock.app.stagePane.costumes.length;
				s = eBlock.app.stagePane.costumes[n].costumeName;
			}
		}
		...
		var receivers:Array = [];
		eBlock.app.stagePane.showCostumeNamed(s);
		eBlock.app.runtime.allStacksAndOwnersDo(findSceneHats);
		var threadList:Array = [];
		for each(var item:Array in receivers){
			threadList.push(eBlock.app.interp.toggleThread(item[0], item[1]));
		}
		return threadList;
	}

	private function backdropNameAt(i:int):String {
		var costumes:Array = eBlock.app.stagePane.costumes;
	...
	private function primChangeEffect(thread:Thread, argList:Array):void {
		...
		s.applyFilters();
		if (s.visible || s == eBlock.app.stagePane) thread.requestRedraw();
	}
	...
	private function minSpriteLayer():int {
		...
		var stg:ScratchStage = eBlock.app.stagePane;
	...
	private function primSetVideoState(thread:Thread, argList:Array):void {
		eBlock.app.stagePane.setVideoState(argList[0]);
	}

	private function primSetVideoTransparency(thread:Thread, argList:Array):void {
		eBlock.app.stagePane.setVideoTransparency(Number(argList[0]));
		eBlock.app.stagePane.setVideoState('on');
	}
