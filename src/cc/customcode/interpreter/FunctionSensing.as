...
package cc.customcode.interpreter {
	...
	public function addPrimsTo(provider:FunctionProvider):void {
		...
		provider.register('answer', function(thread:Thread, argList:Array):void { thread.push( eBlock.app.runtime.lastAnswer) });

		provider.register('mousePressed', function(thread:Thread, argList:Array):void { thread.push( eBlock.app.gh.mouseIsDown) });
		provider.register('mouseX', function(thread:Thread, argList:Array):void { thread.push( eBlock.app.stagePane.scratchMouseX()) });
		provider.register('mouseY', function(thread:Thread, argList:Array):void { thread.push( eBlock.app.stagePane.scratchMouseY()) });
		provider.register('timer', function(thread:Thread, argList:Array):void { thread.push( eBlock.app.runtime.timer()) });
		provider.register('timerReset', function(thread:Thread, argList:Array):void { eBlock.app.runtime.timerReset() });
		...
		provider.register('soundLevel', function(thread:Thread, argList:Array):void { thread.push( eBlock.app.runtime.soundLevel()) });
		provider.register('isLoud', function(thread:Thread, argList:Array):void { thread.push( eBlock.app.runtime.isLoud() )});
		provider.register('timestamp', primTimestamp);
		provider.register('timeAndDate', function(thread:Thread, argList:Array):void { thread.push( eBlock.app.runtime.getTimeString(argList[0])) });

		// sensor
		provider.register('sensor:', function(thread:Thread, argList:Array):void { thread.push( eBlock.app.runtime.getSensor(argList[0])) });
		provider.register('sensorPressed:', function(thread:Thread, argList:Array):void { thread.push( eBlock.app.runtime.getBooleanSensor(argList[0])) });
...
//		if(true || !app.isIn3D) {
			...
			for each (s2 in eBlock.app.stagePane.spritesAndClonesNamed(arg))
...
//		testBM.bitmapData = srcBM;
	...
	private function stageBitmapWithoutSpriteFilteredByColor(s:ScratchSprite, c:int):BitmapData {
		return eBlock.app.stagePane.getBitmapWithoutSpriteFilteredByColor(s, c);
	}

	private function primAsk(thread:Thread, argList:Array):void {
		if (eBlock.app.runtime.askPromptShowing()) {
			thread.suspend();
...
//		if (interp.activeThread.firstTime) {
			...
			if ((obj is ScratchSprite) && (obj.visible)) {
				...
				eBlock.app.runtime.showAskPrompt('');
			} else {
				eBlock.app.runtime.showAskPrompt(question);
			}
			thread.suspend();
			eBlock.app.runtime.askPromptHideSignal.add(thread.resume, true);
...
//		}
	...
	private function primKeyPressed(thread:Thread, argList:Array):void {
		...
		if (key == 'space') ch = 32;
		thread.push( eBlock.app.runtime.keyIsDown[ch]);
	}
...
//		var s:ScratchSprite = thread.userData;
		...
		var obj:ScratchObj = eBlock.app.stagePane.objNamed(argList[1]);
	...
	private function mouseOrSpritePosition(arg:String):Point {
		if (arg == '_mouse_') {
			var w:ScratchStage = eBlock.app.stagePane;
			return new Point(w.scratchMouseX(), w.scratchMouseY());
		} else {
			var s:ScratchSprite = eBlock.app.stagePane.spriteNamed(arg);
	...
	private function primShowWatcher(thread:Thread, argList:Array):void {
		var obj:ScratchObj = ThreadUserData.getScratchObj(thread);
		if (obj) eBlock.app.runtime.showVarOrListFor(argList[0], false, obj);
	}

	private function primHideWatcher(thread:Thread, argList:Array):void {
		var obj:ScratchObj = ThreadUserData.getScratchObj(thread);
		if (obj) eBlock.app.runtime.hideVarOrListFor(argList[0], false, obj);
	}

	private function primShowListWatcher(thread:Thread, argList:Array):void {
		var obj:ScratchObj = ThreadUserData.getScratchObj(thread);
		if (obj) eBlock.app.runtime.showVarOrListFor(argList[0], true, obj);
	}

	private function primHideListWatcher(thread:Thread, argList:Array):void {
		var obj:ScratchObj = ThreadUserData.getScratchObj(thread);
		if (obj) eBlock.app.runtime.hideVarOrListFor(argList[0], true, obj);
	}
