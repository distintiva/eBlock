package cc.customcode.interpreter
{
	...
	internal class PrimInit
	{
		...
		{
			if(!eBlock.app.interp.turboMode){
...
//			if (target.activeThread.firstTime) {
			...
			eBlock.app.runtime.allStacksAndOwnersDo(findReceivers);
			var threadList:Array = [];
			for each(var item:Array in receivers){
				var newThread:Thread = eBlock.app.interp.toggleThread(item[0], item[1]);
