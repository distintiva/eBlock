...
//		private var frame:UpdateFrame;
		...
		*/
		...
		private function __onLoad(evt:Event):void
		{
			...
			var version:Object = util.JSON.parse(str);
			
			if(isSourceVerGreatThan(version.v, eBlock.versionString.slice(1)) ){
				eBlock.app.setInfoText("New version avaliable " + version.v);
			}else{
				eBlock.app.setInfoText("Your version is Up to Date");
			}
			...
			if(isSourceVerGreatThan(result[1], dsBlock.versionString.slice(1)) && (needNotice || SharedObjectManager.sharedManager().getObject(_key, true))){
...
//			closeAndNotify();
		...
		public function start(needNotice:Boolean=false):void
		{
			...
			eBlock.app.setInfoText("Checking for updates ...");
