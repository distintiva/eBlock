...
public class MediaLibrary extends Sprite {
	...
	public function MediaLibrary(app:eBlock, type:String, whenDone:Function) {
		this.app = app;
		this.assetType = type;
...
//		}
	...
	private function addScratchExtensions():void {
		...
		*/
		...
		const extList:Array = [
			{"name":"Nueva", "image":"/devices/bbc_microbit/image.png"},{"name":"Nueva2", "md5":"\image\asas.jpg"}
			];
		
		allItems = [];
		for each (var ext:Object in extList) {
	...
	private function convertAndUploadImageOrSprite(fName:String, data:ByteArray):void {
		...
		function decodeError():void {
			DialogBox.notify('Error decoding image', 'Sorry, Scratch was unable to load the image '+fName+'.', eBlock.app.stage);
		}
		var costumeOrSprite:*;
		var fExt:String = '';
