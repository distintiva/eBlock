...
public class MediaLibraryItem extends Sprite {
	...
	public function MediaLibraryItem(dbObject:Object = null) {
		...
		if (dbObj.tipo =="board" ) isBoard = true;
...
//		}
		...
		if (fileType(md5) == 'svg'){
			var data:ByteArray = eBlock.app.server.getAsset(md5);
			if (data) {
				importer = new SVGImporter(XML(data));
				importer.loadAllImages(svgImagesLoaded);
			}
			...
			eBlock.app.server.getThumbnail(md5, thumbnailWidth, thumbnailHeight, setThumbnail);
		}
	...
	private function setSpriteThumbnail(done:Function):void {
		...
		if (spriteCache[spriteMD5]) {
			...
			data = eBlock.app.server.getAsset(spriteMD5).toString();
		}
	...
	private function addInfoButton():void {
		infoButton = new IconButton(toggleSoundPlay, 'spriteInfo');
		infoButton.x = 75;
		infoButton.y = 28;
		addChild(infoButton);
	}
	...
	private function downloadAndPlay():void {
		...
		var wavData:ByteArray = eBlock.app.server.getAsset(dbObj.md5);
		if (!wavData) return;
		sndData = wavData;
		startPlayingSound();
	}
