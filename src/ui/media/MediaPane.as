...
public class MediaPane extends ScrollFrameContents {
	...
	public function MediaPane(app:eBlock, type:String):void {
		this.app = app;
		isSound = (type == 'sounds');
		refresh();
	}
	...
	private function costumeItems():Array {
		...
		for each (var c:ScratchCostume in viewedObj.costumes) {
			result.push(eBlock.app.createMediaInfo(c, viewedObj));
		}
		return result;
	}
	...
	private function soundItems():Array {
		...
		for each (var snd:ScratchSound in viewedObj.sounds) {
			result.push(eBlock.app.createMediaInfo(snd, viewedObj));
		}
		return result;
	}
