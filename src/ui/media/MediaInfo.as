...
public class MediaInfo extends Sprite {
	...
	public function objToGrab(evt:MouseEvent):* {
		var result:MediaInfo = eBlock.app.createMediaInfo({
			type: objType,
			name: objName,
			md5: md5
		});
		if (mycostume) result = eBlock.app.createMediaInfo(mycostume, owner);
		if (mysound) result = eBlock.app.createMediaInfo(mysound, owner);
		if (mysprite) result = eBlock.app.createMediaInfo(mysprite);
		if (scripts) result = eBlock.app.createMediaInfo(scripts);
	...
	public function handleTool(tool:String, evt:MouseEvent):void {
		...
		if (tool == 'help') eBlock.app.showTip('scratchUI');	}
	...
	protected function duplicateMe():void {
		if (owner) {
			if (mycostume) eBlock.app.addCostume(mycostume.duplicate());
			if (mysound) eBlock.app.addSound(mysound.duplicate());
		}
	}

	protected function deleteMe(ignore:* = null):void {
		if (owner) {
			eBlock.app.runtime.recordForUndelete(this, 0, 0, 0, owner);
			if (mycostume) {
				owner.deleteCostume(mycostume);
				eBlock.app.refreshImageTab(false);
			}
			if (mysound) {
				owner.deleteSound(mysound);
				eBlock.app.refreshSoundTab()
			}
		}
	}
