...
public class ScratchStage extends ScratchObj {
	...
	public function spritesAndClonesNamed(spriteName:String):Array {
		...
		var app:eBlock = parent as eBlock;
		if (app != null) {
...
//		if(MBlock_mod.app.isIn3D) return MBlock_mod.app.render3D.getUIContainer();
		return this;
	}

	override protected function updateImage():void {
		super.updateImage();
//		if(MBlock_mod.app.isIn3D)
//			MBlock_mod.app.render3D.getUIContainer().transform.matrix = transform.matrix.clone();
...
//			if(MBlock_mod.app.isIn3D) MBlock_mod.app.render3D.updateRender(videoImage);
		}
...
//			MBlock_mod.app.render3D.updateRender(penLayer);
...
//		if(!MBlock_mod.app.render3D || s.width < 1 || s.height < 1) return null;
		return null;
/*
		...
		var bmd:BitmapData = MBlock_mod.app.render3D.getRenderedChild(s, b.width*s.scaleX, b.height*s.scaleY, for_carry);
	...
	public function setVideoState(newState:String):void {
		...
		eBlock.app.libraryPart.showVideoButton();
	...
	public function clearPenStrokes():void {
		...
		penActivity = false;
//		if(MBlock_mod.app.isIn3D) MBlock_mod.app.render3D.updateRender(penLayer);
	}
...
//		if(MBlock_mod.app.isIn3D) MBlock_mod.app.render3D.updateFilters(spr, effects);
	}
	...
	public function getBitmapWithoutSpriteFilteredByColor(s:ScratchSprite, c:int):BitmapData {
		...
		if(MBlock_mod.app.isIn3D) {
			var b:Rectangle = s.currentCostume().bitmap ? s.img.getChildAt(0).getBounds(s) : s.getVisibleBounds(s);
			bm1 = MBlock_mod.app.render3D.getOtherRenderedChildren(s, 1);
			//mask = 0x80F8F8F0;
		}
		else {*/
			// OLD code here
...
//		if(MBlock_mod.app.isIn3D) MBlock_mod.app.render3D.updateRender(dispObj, renderID, renderOpts);
	}
...
//		if(MBlock_mod.app.isIn3D) MBlock_mod.app.render3D.getRender(bm);
//		else
		bm.draw(this);
	...
	public function updateInfo():void {
		...
		if (eBlock.app.projectID != '') info.projectID = eBlock.app.projectID;
		info.videoOn = isVideoOn();
		info.swfVersion = eBlock.versionString;

		delete info.loadInProgress;
		if (eBlock.app.loadInProgress) info.loadInProgress = true; // log flag for debugging
	...
	public function handleDrop(obj:*):Boolean {
		if ((obj is ScratchSprite) || (obj is Watcher) || (obj is ListWatcher)) {
			...
			if (!(obj is ScratchSprite) || eBlock.app.editMode) eBlock.app.setSaveNeeded();
			return true;
		}
		eBlock.app.setSaveNeeded();
