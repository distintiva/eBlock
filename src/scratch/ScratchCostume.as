...
public class ScratchCostume {
	...
	public function setBitmapData(bm:BitmapData, centerX:int, centerY:int):void {
		...
		rotationCenterX = centerX;
		rotationCenterY = centerY;
		if (eBlock.app && eBlock.app.viewedObj() && (eBlock.app.viewedObj().currentCostume() == this)) {
			eBlock.app.viewedObj().updateCostume();
			eBlock.app.refreshImageTab(true);
		}
	}
	...
	public function setSVGData(data:ByteArray, computeCenter:Boolean, fromEditor:Boolean = true):void {
		...
		function refreshAfterImagesLoaded():void {
			...
			if (eBlock.app && eBlock.app.viewedObj() && (eBlock.app.viewedObj().currentCostume() == thisC)) {
				eBlock.app.viewedObj().updateCostume();
				eBlock.app.refreshImageTab(fromEditor);
			}
			svgLoading = false;
		}
...
//trace('found bounds: '+new Rectangle(minX, minY, maxX - minX, maxY - minY));
	...
	public function bitmapForEditor(forStage:Boolean):BitmapData {
		...
		var oldQuality:String = eBlock.app.stage.quality;
		eBlock.app.stage.quality = StageQuality.LOW;
		bm.draw(dispObj, m);
		eBlock.app.stage.quality = oldQuality;
