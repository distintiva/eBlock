...
package scratch {
	...
	import cc.customcode.uibot.uiwidgets.lightSetter.LightSetterFrame;
	import cc.customcode.uibot.uiwidgets.lightSetter.LightSetterFrame2;
...
public class BlockMenus implements DragClient {
	...
	public static function BlockMenuHandler(evt:MouseEvent, block:Block, blockArg:BlockArg = null, menuName:String = null):void {
		...
		if (menuName == 'drawPixels'){
			menuHandler.drawFace2(evt);
			return;
		}
...
//		MBlock_mod.app.runtime.checkForGraphicEffects();
	}
			...
			if (!block.isProcDef()) {
				if(block.op.indexOf("runArduino")>-1){
					...
					if(!app.stageIsArduino){
						MenuUtil.AddItem(m, 'upload to board');
...
//			if (block.op != Specs.GET_VAR) return;
	...
	static private var faceFrame2:LightSetterFrame2;
	
	
	private function drawFace(evt:MouseEvent):void
	{
		//-*JC* no dejar editar si no está en ScriptsPane
		if(! (this.block.topBlock().parent is ScriptsPane) ) return;
	...
	private function drawFace2(evt:MouseEvent):void
	{
		...
		if(null == faceFrame2){
			faceFrame2 = new LightSetterFrame2();
		}
		faceFrame2.init(blockArg.base.bmd);
		faceFrame2.addEventListener(AWEvent.HIDDEN, __onFrame2Close);
		faceFrame2.addEventListener(Event.COMPLETE, __onFrame2Select);
		faceFrame2.show();
	}
	...
	private function __onFrameSelect(evt:Event):void
	{
		...
		if(this.app.stageIsArduino){
			this.app.scriptsPart.showArduinoCode();
		}
		
	}
	
	private function __onFrame2Close(evt:AWEvent):void
	{
		faceFrame2.removeEventListener(AWEvent.HIDDEN, __onFrame2Close);
		faceFrame2.removeEventListener(Event.COMPLETE, __onFrame2Select);
	}
	
	private function __onFrame2Select(evt:Event):void
	{
		blockArg.argValue = faceFrame2.getValue();
		blockArg.base.bmd = faceFrame2.getBitmapData();
		blockArg.base.setColor(0);
		blockArg.base.redraw();
