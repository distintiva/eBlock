...
//		var b:Rectangle = (!MBlock_mod.app.render3D || currentCostume().bitmap) ? img.getChildAt(0).getBounds(this) : getVisibleBounds(this);
		...
		if(MBlock_mod.app.isIn3D) {
			...
			updateEffects();
			var bm:BitmapData = MBlock_mod.app.render3D.getRenderedChild(this, b.width*scaleX, b.height*scaleY);
...
//		}
	...
	public function handleTool(tool:String, evt:MouseEvent):void {
		...
		//if (tool == 'help') MBlock_mod.app.showTip('scratchUI');
	}

	private function growSprite():void { setSize(getSize() + 5); eBlock.app.updatePalette() }
	private function shrinkSprite():void { setSize(getSize() - 5); eBlock.app.updatePalette() }
...
//			}
	...
	private function saveToLocalFile():void {
		function success():void {
			eBlock.app.log('sprite saved to file: ' + file.name);
		}
		var zipData:ByteArray = new ProjectIO(eBlock.app).encodeSpriteAsZipFile(copyToShare());
	...
	private function bubbleRect():Rectangle {
		...
		// compute bounds
		var p:Point = eBlock.app.stagePane.globalToLocal(localToGlobal(new Point(0, 0)));
		if (cachedBounds == null) bitmap(); // computes cached bounds
