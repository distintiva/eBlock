...
package scratch {
	...
	import cc.customcode.interpreter.FunctionVideoMotion;
	...
	//	public const emotionResultReceived:Signal = new Signal(Boolean);
		...
		private function __onDeviceEvent(callBackId:int, value:int):void
		{

			allStacksAndOwnersDo(function(stack:Block, target:ScratchObj):void{
				
							
				if( eBlock.app.extensionManager.isCallbackForBlock(callBackId, stack)  ){
...
//			app.oldWebsiteURL = '';
			...
			io.fetchSprite("mdog.json", whenDone);
		}
...
//				if(MBlock_mod.app.isIn3D) obj.updateCostume();
